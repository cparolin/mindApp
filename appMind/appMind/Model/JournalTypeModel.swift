...
class JournalTypeModel {
    ...
    init(type: String, symbol: String, questions: [String]) {
        self.type = type
        self.symbol = symbol
        self.questions = questions
    }
    
    static func initialJournalsCreation(journals: [JournalTypeModel], context: ModelContext) {
        if journals.isEmpty {
            let routine: JournalTypeModel = JournalTypeModel(type: "Rotina", symbol: "arrow.trianglehead.clockwise", questions: [
            ...
            let addiction = JournalTypeModel(type: "Vícios", symbol: "brain.head.profile", questions: [
